__title__ = "Gather all Plumbing Fixtures"
__doc__ = "This add-in uses the FilteredElementCollector with the OfCategory() method to gather all the Plumbing Fixtures in the project"
...
#Then we can apply different modifiers to get what we want. In this class, we can find all the Plumbing Fixtures in the project by using the OfCategory() method and providing the BuiltInCategory.OST_PlumbingFixtures argument
elements = fec.OfCategory(BuiltInCategory.OST_PlumbingFixtures).WhereElementIsNotElementType()
